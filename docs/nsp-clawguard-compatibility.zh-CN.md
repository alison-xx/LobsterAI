...
- 仅修复 `require` 后，macOS 上的 `gateway_start` 回调仍会因内嵌 sql.js
  使用未定义的 `__dirname` 而崩溃，网关刚报告 ready 就退出。

LobsterAI 在配置同步开始时、调用 OpenClaw 迁移 CLI 之前，检查已启用的用户插件，
将该插件已知的 `__require` helper 替换为
`createRequire(import.meta.url)`，并使用 `fileURLToPath(import.meta.url)` 和
`dirname()` 定义模块内的 `__filename`、`__dirname`。v2 同时识别原始 helper
和已应用的 v1 补丁，支持原地升级。其他依赖代码、插件配置和权限保持原样。
...
原文件备份位于入口旁，格式为
`index.mjs.lobsterai-node-globals-v2.<替换前文件 SHA-256>.bak`。
此前的 `index.mjs.lobsterai-native-require-v1.*.bak` 备份继续保留。
...
插件注册与宿主文件关闭，以及异步启动回调的文件全局变量、v1 升级、含空格及
中文的路径。新增变量仅属于插件模块，不写入 `globalThis`。

macOS Node 24.17.0 的隔离回归另外使用本机真实 `2.5.0` 包内未经修改的
sql.js 和内嵌 WASM 片段：v1 复现 `__dirname is not defined`；补齐文件全局
变量后，WASM 初始化、建表写入、保存及重开数据库、`integrity_check` 均通过。
该回归只提取数据库依赖，限制文件访问到临时目录并阻止 fetch，没有执行扫描、
后台服务或远程上报。
...
2026-09-17 的 macOS 开发模式补充回测启用了真实 `gateway_start`：应用 v2
后插件完成数据库初始化及启动检查，网关连续运行超过 60 秒，未退出或自动重启。
插件自身的更新程序随后写入 `2.5.3`，该版本的包头已原生提供 `require`、
`__filename` 和 `__dirname`；LobsterAI 的版本限定补丁不修改这个新版本。

网关监督器同时保留短时间崩溃的重试计数：进程就绪至少 30 秒后才重新获得预算，
连续崩溃用尽 5 次自动重试后，后台重连不能重新启动进程；显式重启或修复可重试。
