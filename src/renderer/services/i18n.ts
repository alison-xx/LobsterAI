...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    coworkModelAutoName: 'Auto',
    coworkModelAutoDescription: '按任务为每一轮自动选择模型：带图片用视觉模型，超长输入用长上下文模型，代码任务用代码模型，其余用通用模型。',
    coworkModelAutoResolvedFormat: '本轮：{model}',
    coworkModelMaxName: 'Max',
    coworkModelMaxDescription: '始终使用 {model}',
    coworkModelMaxUnavailable: '未配置 Max 模型',
    coworkAutoRoutingTitle: 'Auto / Max 模型',
    coworkAutoRoutingDescription: '在对话中选择 Auto 后，每一轮按任务类型自动选择模型。保持「自动」时：通用使用当前智能体的模型，视觉使用可读图的模型，长上下文使用上下文窗口更大的模型，代码使用通用模型。',
    coworkAutoRoutingNeedsModels: '至少需要两个可用模型，Auto 才会出现在模型选择器中。',
    coworkAutoRoutingGeneral: '通用',
    coworkAutoRoutingCode: '代码',
    coworkAutoRoutingVision: '视觉（图片）',
    coworkAutoRoutingLongContext: '长上下文',
    coworkAutoRoutingAutomatic: '自动',
    coworkAutoRoutingMaxDescription: '开启 Max 后，对话的每一轮都使用这里选择的模型；关闭即恢复原来的选择。未选择模型时，模型选择器中不显示 Max。',
    coworkAutoRoutingMaxModel: 'Max 模型',
    coworkAutoRoutingMaxOff: '不使用',
    coworkAutoRoutingUnavailableModel: '{model}（不可用）',
  ...
  en: {
    ...
    coworkModelAutoName: 'Auto',
    coworkModelAutoDescription: 'Picks a model for every turn: images go to a vision model, very long input to a long-context model, code to the code model, and everything else to the general model.',
    coworkModelAutoResolvedFormat: 'This turn: {model}',
    coworkModelMaxName: 'Max',
    coworkModelMaxDescription: 'Always use {model}',
    coworkModelMaxUnavailable: 'No Max model configured',
    coworkAutoRoutingTitle: 'Auto / Max models',
    coworkAutoRoutingDescription: 'When Auto is selected in a conversation, every turn picks a model by task type. Left on Automatic, general uses the agent\'s model, vision uses an image-capable model, long context uses a model with a larger context window, and code uses the general model.',
    coworkAutoRoutingNeedsModels: 'Auto appears in the model selector once at least two models are available.',
    coworkAutoRoutingGeneral: 'General',
    coworkAutoRoutingCode: 'Code',
    coworkAutoRoutingVision: 'Vision (images)',
    coworkAutoRoutingLongContext: 'Long context',
    coworkAutoRoutingAutomatic: 'Automatic',
    coworkAutoRoutingMaxDescription: 'With Max on, every turn in the conversation runs on the model chosen here; turning it off restores the previous selection. Max is hidden in the model selector until a model is chosen.',
    coworkAutoRoutingMaxModel: 'Max model',
    coworkAutoRoutingMaxOff: 'Not set',
    coworkAutoRoutingUnavailableModel: '{model} (unavailable)',
