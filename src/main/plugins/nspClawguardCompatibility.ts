...
const NODE_GLOBALS_V2 = `// LobsterAI: nsp-clawguard 2.5.0 Node.js compatibility v2.
import { createRequire as __lobsteraiNspCreateRequire } from 'node:module';
...
function patchInstalledPlugin(pluginDir: string): boolean {
  ...
  const nodeGlobals = NODE_GLOBALS_V2.replace(/\n/g, newline);
  if (source.includes(nodeGlobals) && !source.includes(legacyRequire) && !source.includes(nativeRequireV1)) return false;

  const previousHelper = source.includes(nativeRequireV1) ? nativeRequireV1 : legacyRequire;
  const offset = source.indexOf(previousHelper);
  if (
    offset < 0 || offset > 1024
    || source.indexOf(previousHelper, offset + previousHelper.length) !== -1
    || (source.includes(nativeRequireV1) && source.includes(legacyRequire))
    || (previousHelper === legacyRequire && source.includes('__lobsteraiNspCreateRequire'))
    || source.includes('__lobsteraiNspDirname')
    || source.includes('__lobsteraiNspFileURLToPath')
  ) {
  ...
  // The bundled sql.js startup also needs CommonJS file globals in native ESM.
  const patched = source.replace(previousHelper, nodeGlobals);
  const hash = createHash('sha256').update(original).digest('hex');
  const backupPath = `${entryPath}.lobsterai-node-globals-v2.${hash}.bak`;
  ...
  safelyReplaceTextFileSync({ filePath: entryPath, content: patched, mode, tempLabel: 'nsp-node-globals' });
  console.log(`[PluginCompatibility] Patched ${NSP_CLAWGUARD.Id} ${pkg.version} Node.js compatibility; backup=${backupPath}`);
  return true;
}
