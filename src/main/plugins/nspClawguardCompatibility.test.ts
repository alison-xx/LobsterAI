...
test.each([false, true])('initializes an async startup hook with Node file globals (previous v1 patch: %s)', previousPatch => {
  const root = path.join(createRoot(), 'plugin space 空间');
  const entry = createPlugin(root);
  let source = pluginSource.replace(
    "export default function register(api) { api.on('before_tool_call', () => {}); }",
    `export default function register(api) {
      api.on('gateway_start', async () => {
        await Promise.resolve();
        // Same unguarded Node branch used by the plugin's bundled sql.js.
        const directory = __dirname + '/';
        return { directory, filename: __filename, exists: __require('fs').existsSync(__filename) };
      });
    }`,
  );
  if (previousPatch) {
    source = source.replace(source.slice(0, source.indexOf('\nconst fs =')), `// LobsterAI: nsp-clawguard 2.5.0 native require compatibility v1.
...
var __require = __lobsteraiNspCreateRequire(import.meta.url);`);
  ...
  const runStartup = () => execFileSync(process.execPath, ['--input-type=module', '--eval', `
    void (async () => {
      ...
      const result = await hooks.get('gateway_start')();
      console.log(JSON.stringify({ ...result, leakedGlobals: '__dirname' in globalThis || '__filename' in globalThis }));
    })().catch(error => { console.error(error); process.exitCode = 1; });
  `], { encoding: 'utf8', windowsHide: true, stdio: 'pipe', timeout: 10_000 });

  expect(() => runStartup()).toThrow(previousPatch ? /__dirname is not defined/ : /Dynamic require/);
  expect(patch(root)).toBe(true);
  const resolvedEntry = fs.realpathSync(entry);
  expect(JSON.parse(runStartup())).toEqual({
    directory: `${path.dirname(resolvedEntry)}/`, filename: resolvedEntry, exists: true, leakedGlobals: false,
  });
  const backup = fs.readdirSync(path.dirname(entry)).find(name => name.endsWith('.bak'))!;
  expect(fs.readFileSync(path.join(path.dirname(entry), backup), 'utf8')).toBe(source);
  const patchedStat = fs.statSync(entry);
  expect(patch(root)).toBe(false);
  expect(fs.statSync(entry).mtimeMs).toBe(patchedStat.mtimeMs);
});
